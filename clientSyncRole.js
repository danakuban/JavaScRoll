...
ClientSyncRoleWrite.observer = new MutationObserver(function (mutations) {
    ...
    mutations.forEach(mutation => {
        ...
        let index;
        if (mutation.target === window.document.getElementById(ClientSyncRoleWrite.entryPoint)) return; // updated by this code
        ...
        if (mutation.type === "childList" && mutation.addedNodes.length > 0) {
            ...
            let node = addedNode;
            while ((node = node.parentNode) && (node.id !== ClientSyncRoleWrite.entryPoint)) {
                path.unshift(node.id, "children");
                if (node.parentNode != null && node.parentNode.id !== ClientSyncRoleWrite.entryPoint) {
        ...
        if (mutation.type === "attributes") {
            ...
            console.log(ClientSyncRoleWrite.entryPoint);
            while ((node = node.parentNode) && (node.id !== ClientSyncRoleWrite.entryPoint)) {
                path.unshift(node.id, "children");
                if (node.parentNode != null && node.parentNode.id !== ClientSyncRoleWrite.entryPoint) path.unshift(getIndex(node));
            }
...
ClientSyncRoleWrite.postPlayed = function() {
    ...
    this.docJson.subscribe(function (err) {
        ...
        that.observer.observe(document.getElementById("entryPoint"), {
            subtree: true,
            attributes: true,
            characterData: true,
            childList: true
